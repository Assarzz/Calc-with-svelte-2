...
function handleClickEvent(event){
    ...
    if (event.detail.type == "number"){
        ...
        Equation.calcArray.push(new ArrayItem(event.detail.type, event.detail.value, parseFloat(event.detail.value) ))
    }
    else{
        Equation.calcArray.push(new ArrayItem(event.detail.type, event.detail.value ))
    }
...
function calculate(){
    ...
    if (this.calcArray[this.calcArray.length].type == "operator"){
        return false
    }
    ...
    function multiplicationAndDivision(){
        for (let index = 0; index < this.calcArray.length; index++) {
            ...
            if (this.calcArray[index].stringValue == "*"){
                ...
                newNumberValue = (this.calcArray[index - 1].intNumberValue * this.calcArray[index + 1].intNumberValue)
                newValue = new ArrayItem("number", newNumberValue.toString(), newNumberValue );
...
<div>
    ...
    <Display/>
    <Keyboard {clicks} {updateClicks} on:buttonclick={(event)=> handleClickEvent(event)} />
